...
public class KP_GA_R {
    ...
    public KP_GA_R(int s, int l, int d, int g, float c, float m, float h) {
        scale = s;
        LL = l;
        dimension = d;
        MAX_GEN = g;
    ...
    public KP_GA_R(int s, IntegerTestCase tc, int g, float c, float m, float h, String test_case_name) {
        scale = s;
        LL = tc.LL;
    ...
    public KP_GA_R(int s, IntegerTestCase tc, int g, float c, float m, float h,
    ...
    public boolean solve() throws IOException {
        ...
        while (t < MAX_GEN && equal_count < 5) {
            ...
            // 随机贪婪局部搜索模块
            //for (i = 0; i < scale; i++) {
            //    fitness[i] = op_local_search(oldPopulation[i], fitness[i], backpack[i], 100);
            //}
            //sortPopulationByFitness(oldPopulation, fitness, backpack);

            // SA局部搜索模块
            //for (i = 0; i < scale; i++) {
            //    fitness[i] = op_sa_local_search(oldPopulation[i], fitness[i], backpack[i], 100,
    ...
    public static void main(String[] args) throws IOException {
        String[] file_path_list = {
                "testcases/chubeas/OR5x250/OR5x250-0.25_1.dat",
                "testcases/chubeas/OR10x250/OR10x250-0.25_1.dat",
                "testcases/chubeas/OR30x250/OR30x250-0.25_1.dat"};
        int[][] L25_5_6 = {
                {1, 1, 1, 1, 1, 1},
                {1, 2, 3, 4, 5, 2},
                {1, 3, 5, 2, 4, 3},
                ...
                {5, 4, 3, 2, 1, 5},
                {5, 5, 5, 5, 5, 1}};
        int[] scale_list = {20, 40, 60, 80, 100};
        float[] Pc_list = {0.4f, 0.6f, 0.8f, 0.9f, 0.99f};
        float[] Pm_list = {0.001f, 0.01f, 0.05f, 0.1f, 0.2f};

        int f_id;
        int exp_count;
        for (f_id = 0; f_id < file_path_list.length; f_id++) {
            String file_path_arg = file_path_list[f_id];//args[0];
            for (exp_count = 0; exp_count < L25_5_6.length; exp_count++) {
                int scale_arg = scale_list[L25_5_6[exp_count][0]-1];//20;//Integer.valueOf(args[1]);
                float Pc_arg = Pc_list[L25_5_6[exp_count][1]-1];//0.5f;//Float.valueOf(args[2]);
                float Pm_arg = Pm_list[L25_5_6[exp_count][2]-1];//0.1f;//Float.valueOf(args[3]);
                int cross_arg = L25_5_6[exp_count][3];//3;//Integer.valueOf(args[4]);
                int mutate_arg = L25_5_6[exp_count][4];//4;//Integer.valueOf(args[5]);
                int select_arg = L25_5_6[exp_count][5];//3;//Integer.valueOf(args[6]);
                ...
                //KP_GA ga = new KP_GA(20, tc, 5000, 0.5f, 0.1f, 0.5f, test_case_name);
                boolean success;
                for (int i = 0; i < 50; i++) {
                    KP_GA_R ga = new KP_GA_R(scale_arg, tc, 5000, Pc_arg, Pm_arg, 0.5f,

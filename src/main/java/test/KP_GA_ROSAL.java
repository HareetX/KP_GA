...
public class KP_GA_ROSAL {
    ...
    public KP_GA_ROSAL(int s, int l, int d, int g, float c, float m, float h) {
        scale = s;
        LL = l;
        dimension = d;
        MAX_GEN = g;
    ...
    public KP_GA_ROSAL(int s, IntegerTestCase tc, int g, float c, float m, float h, String test_case_name) {
        scale = s;
        LL = tc.LL;
    ...
    public KP_GA_ROSAL(int s, IntegerTestCase tc, int g, float c, float m, float h,
    ...
    public boolean solve(int local_times, int T_arg, float Tf_arg, float k_arg, int exp_id) throws IOException {
        int i;
        int k;
        ...
        String filename = "log" + count + "_"+ exp_id + "_" + test_case + ".txt";
        ...
        // 写入种群平均价值变化曲线
        String avg_filename = "log" + count + "_" + exp_id + "_" + test_case.split(".dat")[0] + ".avg.txt";
        ...
        // 写入种群最佳价值变化曲线
        String max_filename = "log" + count + "_" + exp_id + "_" + test_case.split(".dat")[0] + ".max.txt";
    ...
    public static void main(String[] args) throws IOException {
        ...
        int[] local_times_list = {10, 40, 70, 100};
        int[] T_list = {20, 80, 140, 200};
        float[] Tf_list = {0.001f, 0.01f, 0.1f, 1.0f};
        float[] k_list = {0.80f, 0.90f, 0.98f, 0.99f};
        ...
        for (f_id = 0; f_id < file_path_list.length; f_id++) {
            ...
            int select_arg = 5;
            float Ph_arg = 0.6f;
            for (exp_count = 0; exp_count < L16_4_4.length; exp_count++) {
                int local_times_arg = local_times_list[L16_4_4[exp_count][0]-1];
                int T_arg = T_list[L16_4_4[exp_count][1]-1];
                float Tf_arg = Tf_list[L16_4_4[exp_count][2]-1];
                float k_arg = k_list[L16_4_4[exp_count][3]-1];
                ...
                boolean success;
                for (int i = 0; i < 50; i++) {
                    KP_GA_ROSAL ga = new KP_GA_ROSAL(scale_arg, tc, 5000, Pc_arg, Pm_arg, Ph_arg,
                            cross_arg, mutate_arg, select_arg, test_case_name);
                    ga.init();
                    success = ga.solve(local_times_arg, T_arg, Tf_arg, k_arg, exp_count);
                    if (!success) {
                        break;
                    }
                }

...
public class DoubleTestCase implements TestCase {
    double[] v;
    ...
    public boolean readTestCase(String filepath) throws IOException {
                    ...
                    for (int i = 0; i < len; i++) {
                        if (st_list[i] != null) {
                            if (dimension_count < dimension) {
                                double n = Double.valueOf(st_list[i]);
                                pb[dimension_count] = n;
                                dimension_count += 1;
                            }
                            else if (dimension_count == dimension) {
                                double n = Double.valueOf(st_list[i]);
                                best_v = n;
                            }
                            else {
